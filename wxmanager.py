...
class WXDaySummaryManager(weewx.manager.DaySummaryManager):
    ...
    Like a regular daily summary database, except it understands wind, and heating- growing- and cooling-degree days."""

    # Default base temperature and unit type for heating, coolinga and growing degree days,
    # as a value tuple
    ...
    def getAggregate(self, timespan, obs_type, aggregateType, **option_dict):
        """Specialized version of getDayAggregate that can calculate heating or growing or  cooling degree days.

        timespan: An instance of weeutil.Timespan with the time period over which
        aggregation is to be done.

        obs_type: The observation type. For anything other than 'heatdeg' or 'cooldeg' or 'growdeg',
        the superclass is used.

        aggregateType: The type of aggregation to be done. For 'heatdeg' or 'cooldeg' or 'growdeg',
        ...
        growbase = dd_dict.get('growing_base')
        ...
        growbase_t = (float(growbase[0]), growbase[1], "group_temperature") if growbase else WXDaySummaryManager.default_growbase
        ...
        for daySpan in weeutil.weeutil.genDaySpans(timespan.start, timespan.stop):
            ...
            if Tavg_t is not None and Tavg_t[0] is not None:
                ...
                elif obs_type == 'growdeg':
                    # Convert average temperature to the same units as heatbase:
                    Tavg_target_t = weewx.units.convert(Tavg_t, growbase[1])
                    _sum += weewx.wxformulas.growing_degrees(Tavg_target_t[0], growbase_t[0])
                else:
